...
func generateStructUpdater(structInfo StructInfo) {
	...
	fmt.Println("\t// Make a copy of the updateMap to avoid potential concurrent access issues")
	fmt.Println("\tupdateMapCopy := make(map[string]interface{}, len(updateMap))")
	fmt.Println("\tfor k, v := range updateMap {")
	fmt.Println("\t\tupdateMapCopy[k] = v")
	fmt.Println("\t}")
	fmt.Println("\t// Update only fields that are present in the map using precomputed updaters")
	fmt.Printf("\tfor fieldName, val := range updateMapCopy {\n")
